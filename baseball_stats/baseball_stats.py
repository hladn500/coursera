...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        ...
        tuples.append((player, stat))
    tuples.sort(key=lambda pair: pair[1], reverse=True)
    return [tuples[index] for index in range(numplayers)]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    lookup = read_csv_as_nested_dict(info["masterfile"], info["playerid"],
                                     info["separator"], info["quote"])
    formated_strings = []
    for pair in top_ids_and_stats:
        first_name = lookup[pair[0]][info["firstname"]]
        last_name = lookup[pair[0]][info["lastname"]]
        formated_strings.append(f"{pair[1]:.3f} --- {first_name} {last_name}")
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    statistics = read_csv_as_list_dict(info["battingfile"], info["separator"],
                                       info["quote"])
    filtered = filter_by_year(statistics, year, info["yearid"])
    top_players = top_player_ids(info, filtered, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    aggregated = {}
    for row in statistics:
        if row[playerid] not in aggregated:
            aggregated[row[playerid]] = {key: int(row[key]) for key in fields}
            aggregated[row[playerid]][playerid] = row[playerid]
        else:
            aggregated[row[playerid]] = {key: int(aggregated[row[playerid]][key]) +
                                         int(row[key]) for key in fields}
            aggregated[row[playerid]][playerid] = row[playerid]
    return aggregated
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    statistics = read_csv_as_list_dict(info["battingfile"], info["separator"],
                                       info["quote"])
    aggregated = aggregate_by_player_id(statistics, info["playerid"],
                                        info["battingfields"])
    top_players = top_player_ids(info, aggregated.values(), formula, numplayers)
    return lookup_player_names(info, top_players)
